...
class TabularLayout(colSizes: Array<out SizingRule>, initialRowSizes: Array<out SizingRule>) : LayoutManager2 {
    ...
    data class SizingRule(
        ...
        companion object {
             ...
             * "px" (e.g. "100px") or + "dp" (e.g. "200dp"). "DP" stands for "density-independent pixel" and will be scaled up on Hi-DPI screens.
            ...
            fun fromString(s: String): SizingRule {
                ...
                try {
                    return when {
                        ...
                        s.endsWith("dp") -> SizingRule(Type.FIXED, JBUI.scale(s.dropLast(2).toInt()))
    ...
    /**
     * Create a constraint which can live across multiple cells.
     *
     * Note that components which span across multiple cells aren't included in fit-to-size layout calculations.
     */
    data class Constraint(val row: Int, val col: Int, val rowSpan: Int, val colSpan: Int) {
        /**
     ...
     * - "75*,25*" - Same as above
     * - "50dp,*,100dp" - First cell gets 50 pixels, last cell gets 100, middle gets remaining space
     *
     ...
     */
    private class SizeCalculator(private val rules: List<SizingRule>, private val gap: Int) {
        ...
        init {
            ...
            for ((i, rule) in rules.withIndex()) {
                when (rule.type) {
                    ...
                    SizingRule.Type.FIXED -> sizes[i] = rule.value
                    SizingRule.Type.FIT -> {
                        // do nothing
                    }
                }
